...
/**************************************************************************************************
 * CommonSlider | 레이어를 x축 혹은 y축으로 움직이는 slider입니다.
 *
 * @class CommonSlider
 * @constructor
 * @version 1.0
...
(function($) {
	...
	window.CommonSlider = function(container, options) {
		...
		var container = typeof container === 'object' ? container : $('#' + container),
			opt = {autoPlay: true, axis: 'x', repeat: true, loop: true, lazyLoad: false, itemsPerView: 1, itemsPerMove: 1, startIndex: 0, interval: 6000, duration: 500, easing: 'swing', unit: 'px', pagination: undefined, prevBtn: undefined, nextBtn: undefined, init: undefined, before: undefined, after: undefined};
		...
		if(length - opt.itemsPerView < 1) return false;

		var totalPage, pagingList;
		...
		this.initialize = function() {
			if(length - opt.itemsPerView >= 1) {
				...
				items.css({'float': opt.axis === 'x' ? 'left' : 'none'});
				if(opt.loop === true) {
					items.filter(':gt('+ (length - 1 - opt.itemsPerView) +')').clone().addClass(cloneClass).prependTo(wrapper);
					items.filter(':lt('+ opt.itemsPerView +')').clone().addClass(cloneClass).appendTo(wrapper);
					$('.' + cloneClass, container).each(function() {
						_this.imgLoad($(this));
					});
				}
				var tempTotalPage = Math.ceil((length - opt.itemsPerView) / opt.itemsPerMove) + 1;
				totalPage = opt.loop === true ? tempTotalPage + (opt.itemsPerView - opt.itemsPerMove) : tempTotalPage;
				...
				if(opt.lazyLoad === true) this.lazyLoad(true);
				this.setSliderProps();
				this.setTranslate();
				this.setHandler();
				this.timerCtrl();
			...
			if(typeof opt.init === 'function') opt.init();
		};
		...
		this.setHandler = function() {
			...
			if(typeof opt.pagination === 'object') {
				...
				clearLists.push(pagingList.children());
			}
		...
		this.remHandler = function() {
			...
			if(typeof opt.pagination === 'object') pagingList.children().unbind('click', this.toIdx);
			if(typeof opt.prevBtn === 'object') opt.prevBtn.unbind('click', this.toPrev);
			if(typeof opt.nextBtn === 'object') opt.nextBtn.unbind('click', this.toNext);
		...
		this.toPrev = function() {
			if(tweening === false) {
				...
				index = index === -1 ? totalPage - 1 : index;
				if(opt.lazyLoad === true && imgLoadComplete === false) _this.lazyLoad();
				method = 'toPrev';
				if(index !== prevIndex) _this.slide();
			}
		};
		...
		this.toNext = function() {
			if(tweening === false) {
				index++;
				if(opt.repeat === false && index === totalPage) {
					index--;
					_this.clearTimer();
					return false;
				}
				index = index === totalPage ? 0 : index;
				if(opt.lazyLoad === true && imgLoadComplete === false) _this.lazyLoad();
				method = 'toNext';
				if(index !== prevIndex) _this.slide();
			}
		 ...
		 *
		 * @method toIdx
		 * @param {Object} e 이벤트 객체 혹은 인덱스 번호
		 */
		...
		this.slide = function() {
			...
			var tweenProps = {ease: opt.easing, onComplete: function() {
				_this.setTranslate();
			...
			tweenProps[cssProp] = _this.getTranslate();
			tween = TweenMax.to(wrapper, opt.duration / 1000, tweenProps);
		};
		...
		this.resize = function() {
			...
			resizeTimer = setTimeout(function() {
				...
				if(tweening === true) {
					tween.vars.css[cssProp] = _this.getTranslate();
					tween.invalidate();
				} else {
					_this.setTranslate();
				}
		...
		this.setSliderProps = function() {
			...
			if(opt.itemsPerView > 1) {
				if(opt.unit === 'px') itemsD = opt.axis === 'x' ? items.eq(0).outerWidth(true) : items.eq(0).outerHeight(true);
				else itemsD = itemsD / opt.itemsPerView;
			}
			if(opt.axis === 'x') {
				if(opt.loop === true) wrapper.width(itemsD * (length + (opt.itemsPerView * 2)));
				else wrapper.width(itemsD * length);
				if(opt.itemsPerView === 1 || opt.unit !== 'px') wrapper.children('li').width(itemsD);
			} else {
				if(opt.loop === true) wrapper.height(itemsD * (length + (opt.itemsPerView * 2)));
				else wrapper.height(itemsD * length);
				if(opt.itemsPerView === 1 || opt.unit !== 'px') wrapper.children('li').height(itemsD);
			}
			for(var i=0; i<totalPage; i++) {
				if(opt.loop === true) {
					if(i > 0) diff[i] = diff[0] - (itemsD * opt.itemsPerMove * i);
					else diff[i] = -(itemsD * opt.itemsPerView);
				} else {
					diff[i] = -(itemsD * opt.itemsPerMove * i);
				}
				if(restItems > 0 && i === totalPage - 1) diff[i] = diff[i - 1] - (itemsD * restItems);
            }
		};
		...
		this.getTranslate = function() {
			var value;
			if(opt.loop === true && method === 'toPrev' && index === totalPage - 1) {
				value = diff[0] + (opt.itemsPerMove * itemsD);
			} else if(opt.loop === true && method === 'toNext' && index === 0) {
				value = diff[totalPage - 1] - (opt.itemsPerMove * itemsD);
			} else {
				value = diff[index];
			}
			return value;
		...
		this.setTranslate = function() {
			var tempObj = {};
			tempObj[cssProp] = diff[index];
			TweenMax.set(wrapper, tempObj);
		};

		/*
		 * 해당 영역에 도달하면 이미지를 로드합니다.
		 *
		 * @method lazyLoad
		 * @param {Boolean} init 옵션
		 */
		this.lazyLoad = function(init) {
			if(typeof init === 'boolean' && init === true) {
				var prop = opt.itemsPerView;
				var dist = 1;
			} else {
				var prop = opt.itemsPerMove;
				var dist = Math.abs(index - prevIndex);
			}
			for(var i=index * prop, j=0, idx; i<(index + dist) * opt.itemsPerView; i++) {
				if(i < length) idx = i;
				else {
					idx = j;
					j++;
				}
				if(items.eq(idx).length > 0) this.imgLoad(items.eq(idx));
			}
		};
		 ...
		 *
		 * @method imgLoad
		 * @param {Object} el 이미지의 부모 요소
		 */
		this.imgLoad = function(el) {
			el.data('lazyLoad', true);
...
(function($) {
	...
	window.Popup = function(type, options) {
		...
		this.type03 = function() {
			var container = $('#popupWrap'),
	            containerH = parseInt(container.css('height'), 10),
				fixed = $('.popup_fixed');
			if(typeof opt.callback === 'function' && container.length === 0 && $('[id^=popup]').length === 0) {
				opt.callback();
				return false;
			}
			if(fixed.length > 0) fixed.animate({top: containerH}, 700, 'easeOutExpo');
			container.slideDown(700, 'easeInExpo');
			$('#hpopupX, #hpopcheck').click(function(e) {
                if($(this).is('#hpopcheck') === true) _this.setCookie('hpopup', 'hide', 1);
				if(fixed.length > 0) fixed.animate({top: 0}, 500, 'easeOutExpo');
				container.slideUp(500, 'easeOutExpo', function() {
					$(this).remove();
...
(function($) {
	...
	window.DimensionFix = function($container, options) {
		...
		var opt = {fixElem: undefined, w: 1800, h: 900, hAlign: 'center', vAlign: 'center'};
		...
		this.kill = function() {
			$(window).off('resize', this.setDimension);
		};
	};
...
/**************************************************************************************************
 * MouseWheel | 마우스휠 이벤트를 제어하기 위한 class입니다.
 *
 * @class MouseWheel
 * @constructor
 * @version 1.0
 ...
 *
 **************************************************************************************************/
window.MouseWheel = function(callback) {

	'use strict';

	if(this instanceof MouseWheel === false) {
		return new MouseWheel(callback);
	}

	var _this = this;

    var	eventType, eventMethod;

	/*
	 * MouseWheel class 초기화 함수
	 *
	 * @method initialize
	 */
	this.initialize = function() {
	...
	/*
	 * 브라우저 환경에 따라 이벤트 타입을 조정합니다.
	 *
	 * @method eventDetect
	 */
	this.eventDetect = function() {
		if('onwheel' in document) {
			eventType = 'wheel'; // for modern browser
		} else {
			if(typeof window.addEventListener === 'function') {
				if('onmousewheel' in document) eventType = 'mousewheel'; // for older webkit
				else eventType = 'DOMMouseScroll'; // for older firefox
			} else {
				eventType = 'onmousewheel'; // for older IE
				eventMethod = 'attach';
			}
		}
		this.addEvent();
	 ...
	 *
	 * @method handler
	 * @param {Object} 마우스휠 이벤트 객체
	 */
	this.handler = function(e) {
		e = e || window.event;
		typeof e.preventDefault === 'function' ? e.preventDefault() : e.returnValue = false;
		var value = e.deltaY ? e.deltaY : e.wheelDelta ? -e.wheelDelta : e.detail;
		if(value === 0) return false;
		var dir = value > 0 ? 'fore' : 'back';
		typeof callback === 'function' ? callback(e, dir) : null;
	};

	/*
	 * mousewheel event handler를 등록합니다.
	 *
	 * @method addEvent
	 */
	this.addEvent = function() {
		if(typeof eventMethod === 'undefined') window.addEventListener(eventType, _this.handler, false);
		else document.attachEvent(eventType, _this.handler);
	};

	/*
	 * mousewheel event handler를 취소합니다.
	 *
	 * @method remEvent
	 */
	this.remEvent = function() {
		if(typeof eventMethod === 'undefined') window.removeEventListener(eventType, _this.handler, false);
		else document.detachEvent(eventType, _this.handler);
	};

	// MouseWheel class 초기화 함수를 호출합니다.
	this.initialize();
};
...
(function($) {
    ...
    window.PictureViewer = function() {
        ...
        var viewPoint = [
                {dim: ['100%', '100%'], left: {value: 0, backEasing: Power0.easeNone}, top: {value: 0, backEasing: Power0.easeNone}},
                {dim: ['290%', '290%'], left: {value: '-80%', foreEasing: Power0.easeNone, backEasing: Power0.easeNone}, top: {value: '-70%', foreEasing: Power0.easeNone, backEasing: Power0.easeNone}},
                {dim: ['240%', '240%'], left: {value: '0%', foreEasing: Power0.easeNone, backEasing: Power0.easeNone}, top: {value: '-18%', foreEasing: Power0.easeNone, backEasing: Power0.easeNone}},
                {dim: ['200%', '200%'], left: {value: '-0%', foreEasing: Power0.easeNone, backEasing: Power0.easeNone}, top: {value: '-60%', foreEasing: Power0.easeNone, backEasing: Power0.easeNone}},
                {dim: ['150%', '150%'], left: {value: '-25%', foreEasing: Power0.easeNone, backEasing: Power0.easeNone}, top: {value: '-30%', foreEasing: Power0.easeNone, backEasing: Power0.easeNone}},
                // {dim: ['300%', '300%'], left: {value: '-73%', foreEasing: Power0.easeNone}, top: {value: '-84%', foreEasing: Power0.easeNone}}
            ],
            pointLength = viewPoint.length,
            tween = false,
			prevIndex = 0,
            index = 0;
        ...
        this.setViewerHtml = function() {
            html = '';
			html += '<div class="picture-container">\n';
			html += '	 <ul class="picture-tab fs_def t_center">\n';
			html += '	 	<li class="on"><a href="#none">룸타입</a></li>\n';
			html += '	 	<li><a href="#none">홀타입</a></li>\n';
			html += '	 	<li><a href="#none">프라이빗형</a></li>\n';
			html += '	 	<li><a href="#none">오픈주방형</a></li>\n';
			html += '	 </ul>\n';
			html += '	 <div class="picture-tab-conts h100">\n';
			html += '    	<div class="picture-wrapper">\n';
			html += '        	<div class="picture-viewer">\n';
			html += '            	<img src="../img/page/brand/03/img1.jpg" alt="" class="w100">\n';
			html += '        	</div>\n';
			html += '   		<div class="picture-control">\n';
			html += '   			<a href="#none" class="picture-initial">0</a>\n';
			html += '				<ul class="picture-ul">\n';
			html += '					<li class="picture-li01"><a href="#none"><span>테이블 파티션으로<br>독립적인 공간 창출</span></a></li>\n';
			html += '					<li class="picture-li02"><a href="#none"><span>전통 목로 선술집<br>분위기 연출</span></a></li>\n';
			html += '					<li class="picture-li03"><a href="#none"><span>따스한 감성이 묻어난<br>목로의 공간</span></a></li>\n';
			html += '					<li class="picture-li04"><a href="#none"><span>산카이만의<br>  분위기</span></a></li>\n';
			html += '				</ul>\n';
			html += '   			<a href="#none" class="picture-close"><span>닫기</span></a>\n';
			html += '   		</div>\n';
			html += '   	</div>\n';
			html += '    	<div class="picture-wrapper" style="display:none">\n';
			html += '        	<div class="picture-viewer">\n';
			html += '            	<img src="../img/page/brand/03/img2.jpg" alt="" class="w100">\n';
			html += '        	</div>\n';
			html += '   		<div class="picture-control">\n';
			html += '   			<a href="#none" class="picture-initial">0</a>\n';
			html += '				<ul class="picture-ul">\n';
			...
			html += '					<li class="picture-li02"><a href="#none"><span>전통 목로 선술집<br>분위기 연출</span></a></li>\n';
			html += '					<li class="picture-li03"><a href="#none"><span>따스한 감성이 묻어난<br>목로의 공간</span></a></li>\n';
			html += '					<li class="picture-li04"><a href="#none"><span>산카이만의<br>세련된 분위기</span></a></li>\n';
			html += '				</ul>\n';
			html += '   			<a href="#none" class="picture-close"><span>닫기</span></a>\n';
			html += '   		</div>\n';
			html += '   	</div>\n';
			html += '    	<div class="picture-wrapper" style="display:none">\n';
			html += '        	<div class="picture-viewer">\n';
			html += '            	<img src="../img/page/brand/03/img3.jpg" alt="" class="w100">\n';
			html += '        	</div>\n';
			html += '   		<div class="picture-control">\n';
			html += '   			<a href="#none" class="picture-initial">0</a>\n';
			html += '				<ul class="picture-ul">\n';
			...
			html += '    	<div class="picture-wrapper" style="display:none">\n';
			html += '        	<div class="picture-viewer">\n';
			html += '            	<img src="../img/page/brand/03/img4.jpg" alt="" class="w100">\n';
			html += '        	</div>\n';
			html += '   		<div class="picture-control">\n';
			html += '   			<a href="#none" class="picture-initial">0</a>\n';
			html += '				<ul class="picture-ul">\n';
        ...
        this.appendCanvas = function() {
            body.append('<div id="pictureBlockBg" style="position:fixed;left:0;top:0;z-index:2000;width:100%;height:100%;background-color:#000"></div>\n');
            blockBg = $('#pictureBlockBg');
            blockBg.css({opacity: 0.5});
            body.append(html);
            container = $('.picture-container');
            viewport = $('.picture-wrapper');
            viewer = $('.picture-viewer');
			picture = viewer.children('img');
            selectedViewport = $('.picture-tab-conts .picture-wrapper').eq(0);
            new Tabbing($('.picture-tab'), $('.picture-tab-conts'));
            $('.picture-tab li a').click(function(){
                var selectedIndex = $(this).closest('li').index();
                selectedViewport = $('.picture-tab-conts .picture-wrapper').eq(selectedIndex);
                viewport = selectedViewport;
                viewer = $('.picture-viewer', selectedViewport);
    			picture = viewer.children('img');
				tween = false;
				index = 0;
				prevIndex = 1;
        ...
        this.setHandler = function() {
			$('a[href*="brand_3.html"]').click(function(e) {
				if(open === false) {
					_this.activate();
					open = true;
				}
			...
			body.on('click', '.picture-ul > li > a', function(e) {
                if($(this).closest('.picture-wrapper')[0] !== selectedViewport[0]){
                    e.preventDefault();
                    return;
                }
				if(tween === false) {
					...
					_this.zoom(index - prevIndex > 0 ? 'fore' : 'back');
				}
			});
			body.on('click', '.picture-initial', function(e) {
                if($(this).closest('.picture-wrapper')[0] !== selectedViewport[0]){
                    e.preventDefault();
                    return;
                }
            ...
            body.on('click', '#pictureBlockBg', this.kill);
            body.on('click', '.picture-close', this.kill);
        };

        this.zoomHandler = function(e, dir) {
            if(tween === false) {
				tween = true;
                if(dir === 'fore' && index < pointLength - 1) index++;
                else if(dir === 'back' && index > 0) index--;
				_this.zoom(dir);
            }
        };
		...
		this.zoom = function(dir) {
			if(index !== prevIndex) {
				...
				TweenLite.to(picture, animDuration, {width: viewPoint[index]['dim'][0], height: viewPoint[index]['dim'][1], ease: Power0.easeNone});
				var easing = dir === 'fore' ? 'foreEasing' : 'backEasing';
				TweenLite.to(picture, animDuration, {left: viewPoint[index]['left']['value'], ease: viewPoint[index]['left'][easing]});
				TweenLite.to(picture, animDuration, {top: viewPoint[index]['top']['value'], ease: viewPoint[index]['top'][easing], onComplete: function() {
					prevIndex = index;
					tween = false;
				}});
				$('.picture-ul > li', selectedViewport).filter('.on').removeClass('on');
				if(index > 0) {
					$('.picture-ul > li', selectedViewport).eq(index - 1).addClass('on');
				}
			} else {
				tween = false;
			}
		...
		this.activate = function() {
			this.appendCanvas();
			dimFixObj = new DimensionFix(viewport, {
				fixElem: viewer,
				x: imgW,
				y: imgH
			});
			dimFixObj.fix();
			wheelObj = new MouseWheel(this.zoomHandler);
		};
...
(function($) {
    ...
    window.Booklet = function(options) {

        var _this = this;

		var opt = {width: 1000, height: 700, speed: 500, padding: 0, overlays: true, arrows: true, hash: false, scrollBar: true, initTop: 50};

		for(var prop in options) {
			opt[prop] = options[prop];
		}

		var containerId = '#bookletContainer',
			containerH,
        	container,
			anchor = 'a[href*="booklet"]',
			bookletId = '#pageWrap',
			booklet,
			blockBgId = '#blockBg',
            blockBg,
			...
			html = $('html'),
			body = $('body'),
			scrollBar = html.css('overflow') === 'hidden' ? false : true,
			modalTop,
			resizeTimer = null,
			queue = false;

		var	requestPath = CONST_REQUEST_ROOT + '/booklet';
        ...
        this.setHanlder = function() {
			$(blockBgId).click(this.modalX);
			$(xBtnId).click(this.modalX);
			win.resize(this.resize);
        };

		this.remHandler = function() {
			$(blockBgId).unbind('click', this.modalX);
			$(xBtnId).unbind('click', this.modalX);
			win.unbind('resize', this.resize);
		};

		this.modalOpen = function(e) {
			if(queue === false) {
				var startPage = typeof $(this).data('page') === 'number' ? $(this).data('page') : 0;
				$.post(requestPath + '/booklet.html', {path: requestPath, w: opt.width, h: opt.height}, function(response) {
					$('body').append('<div id="blockBg" style="position:fixed;left:0;top:0;z-index:999;width:100%;height:100%;background-color:#000"></div>');
					blockBg = $(blockBgId);
					blockBg.css('opacity', 0.8);
					$('body').append(response);
					container = $(containerId);
					containerH = parseFloat(container.css('height'), 10);
					_this.fixModalCss();
					booklet = $(bookletId);
					booklet.booklet({
        ...
        this.modalX = function(e) {
			if(opt.scrollBar === false && scrollBar === true) html.css('overflow', 'visible');
			booklet.booklet('gotopage', 1);
			booklet.booklet('destroy');
			container.parent('#bookletTotalHtml').remove();
			blockBg.remove();
			_this.remHandler();
            e.preventDefault();
        };
		...
		this.fixModalCss = function() {
			if(win.innerHeight() > containerH) {
				if(opt.scrollBar === false && scrollBar === true) html.css('overflow', 'hidden');
				modalTop = win.scrollTop() + (win.innerHeight() / 2) - (containerH / 2);
			} else {
				if(opt.scrollBar === false && scrollBar === true) html.css('overflow', 'visible');
				modalTop = win.scrollTop() + opt.initTop;
			}
			container.css({top: modalTop});
		};
...
(function($) {
	$(document).ready(function() {
		new Policy();
		// new Tabbing($('.picture-tab'), $('.picture-tab-conts'));
		new Booklet({width: 928, height: 812});
		new PictureViewer();
